...
filenames <- list.files(path='low_copy/',pattern="histogram_pmd_*", full.names=TRUE)
filenames <- list.files(pattern="histogram_pmd_*", full.names=TRUE)
...
ggplot(fd) + geom_density(aes(x=val, colour=cop)) + facet_wrap(~name)+xlim(0,150)
