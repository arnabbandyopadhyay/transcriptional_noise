...
#                print(self.influx)
        ...
        if self.influx>=0.5:
            ...
            if ((switch=='yes') and (random.uniform(0, 1)<prob) and (self.influx<=0.3)):
                nl[0].influx=random.uniform(0.5, 0.6)
                nl[1].influx=random.uniform(0.5, 0.6)#self.influx
#        
#        nl[0].influx=self.influx
...
#            nl[1].factor=np.exp(-(nl[1].mu-30)/50)
        ...
        nl[0].gmax=20+30*np.exp(-0.05*nl[0].pd)
        nl[0].mu=nl[0].gmax
        nl[0].nextdiv=nl[0].birthtime+nl[0].mu
        
        nl[1].gmax=20+30*np.exp(-0.05*nl[1].pd)
        nl[1].mu=nl[1].gmax
