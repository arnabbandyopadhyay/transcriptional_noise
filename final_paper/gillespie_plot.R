...
colnames(data)<-c('param','glpD','glpR')

df_median <- data  %>% group_by(param)%>% summarise(mD=sd(mD)/mean(mD),mR=sd(mR)/mean(mR))
df_median<-melt(df_median,id='param')
df_median$param<-as.factor(df_median$param)
...
ggplot(data, aes(x = param, y = value,color=variable,fill=variable)) +
  ...
  geom_boxplot( fill='white',position = position_dodge(0.9),width=0.1,show.legend=FALSE)+
  geom_boxplot( col='black',position = position_dodge(0.9),width=0.1,alpha=.001,show.legend=FALSE)+
  geom_point(data=df_median,mapping=aes(y = value*coeff),fill='black', position = position_dodge(0.9),size = 4,shape=21)+
  scale_y_continuous(
    sec.axis = sec_axis(~./coeff, name="CV")
                     ...
                     legend.text = element_text( size=30, face="bold"),
                     strip.text.x = element_text(size = 14, color = "black", face = "bold.italic"),
                     strip.text.y = element_text(size = 14, color = "black", face = "bold.italic"))+ylab('mRNA')
