...
random.seed(5)
mus=[random.randrange(60,65) for rn in range(100)]
sigma = 10
...
def main_fn(core):
    ...
    gly_xy=np.random.randint(1000,size=[100000,2])
    g3p_xy=np.random.randint(1000,size=[10000,2])
...
############## uptake of g3p from nearby
            ...
            g3pn=round(np.random.random()*i.pg3p)
            # print(g3pn)
            if g3pn>0:
...
if __name__ == '__main__':
    with Pool(10) as p:
        print(p.map(main_fn, range(1,10)))
